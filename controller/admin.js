...
exports.addPlace = function(req, res){
    ...
    form.parse(req, function (err, fields, files) {
      ...
      console.log(dir);
      PlaceModel.update({name:name}, {$setOnInsert:{name:name, latitude:latitude, longitude:longitude, capacity:capacity, price:price, dir:dir, queue:"0"}}, {upsert:true}).exec(function(err, data){
          if(err)
              res.json({ret:false});
          else{
...
exports.deletePlace = function (req, res){
    ...
    PlaceModel.find({name:name}).remove().exec(function(err, data){
        if(err)
            ...
            UserModel.find({"local.email":name.toLowerCase()+"@owner.com"}).remove().exec(function(err, data){
                if(err){
                    res.json({ret:false});
                }
                else{
